...
if __name__ == "__main__":
    ...
    # training the emission and transmission matrices from 'train_combined'
    xtrain, ytrain = init()
    ...
    transition_matrix, tags_countappearance, tags_counttransition, tags_countzero, unique_tags = producetransitionmatrix_part5('train_combined',dt)

    # predicts the tags for data file 'test.in'
    optimal_tags2, total_string, countphrases, checklen, last_input_string, optimal_tags_error, input_string_error  = producetagging('test.in', transition_matrix, e, yset, xset)
    nospaces2 = []
    for tag in optimal_tags2:
        if tag == ' ':
            pass
        else:
            nospaces2.append(tag)   
    writefile2(nospaces2);
